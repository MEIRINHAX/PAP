...
email.addEventListener("input", function(){
    if (!checkEmail(email.value)) {
        emailError.textContent = "*E-mail não é valido"
    }else {
...
username.addEventListener("input", function(){
    if (username.value.length < 4) {
        usernameError.textContent = "*O nome de usuário deve ter pelo menos 8 caracteres."
    }else if(username.value.length > 20){
        usernameError.textContent = "*O nome de usuário deve ter menos de 20 caracteres.";
    }else {
...
lgPassword.addEventListener("input", function(){
    if (lgPassword.value.length < 8) {
        lgPasswordError.textContent = "*A senha deve ter pelo menos 8 caracteres."
...
function checkRequiredLg(inputArr2) {
    ...
    inputArr2.forEach(function(input){
        if (input.value.trim() === '') {
            showError2(input, `*${getFieldNameLg(input)} Por favor preencha este campo`)
            isRequiredLg = true
        }else {
            showSuccess2(input)
        }
